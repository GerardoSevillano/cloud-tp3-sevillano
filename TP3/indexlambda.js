  ...
  switch (event.httpMethod) {
    ...
    case "GET":
      if (event.resource == '/envios/pendientes') {
        const params = {
          TableName: 'envio',
          IndexName: 'indice_envios_pendientes'
        };
        ...
        const envioID = event.pathParameters.envioID;
        if (envioID) {
            var params = {
              TableName: "envio",
        ...
        break;
    case "POST":
      if (event.resource == "/envios/{envioID}/entregado") {
        const envioID = event.pathParameters.envioID;
        var params = {
          TableName: "envio",
          Key: {
        ...
        docClient.update(params, function(err, data) {
          if (err) callback(null, { body: err });
          // an error occurred
          else callback(null, { body: data.Attributes.id + " Se entrego" }); // successful response
        });
      } else {
        var params = {
          TableName: "envio",
